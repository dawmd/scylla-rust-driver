...
or refrain from preferring datacenters (which may ban all other datacenters, if datacenter failover happens to be not possible)."
        ...
        let replica_order = Self::replica_order(query);
...
impl DefaultPolicy {
    ...
    /// Returns the replica ordering to use for a request.
    ///
    /// LWT statements are routed to replicas in a deterministic order, to reduce the contention
    /// caused by Paxos conflicts. Every other request keeps the arbitrary (spread) ordering.
    ///
    /// Note that leader-aware routing does *not* ask for a deterministic order: it promotes one
    /// specific replica (see [`DefaultPolicy::leader_to_prefer`]) and leaves the ordering of the
    /// rest alone, so retries after the leader still spread.
    fn replica_order(query: &RoutingInfo) -> ReplicaOrder {
        if query.should_route_as_lwt() {
            ReplicaOrder::Deterministic
        } else {
    ...
    /// deterministically to the first replica targets the leader. Two conditions must hold:
    ...
    fn leader_to_prefer<'a>(
        ...
        // The server puts the leader first in a tablet's replica list and `ReplicasOrdered`
        // preserves that order. This is also its cheap case: only the first replica is computed
        // eagerly, which is all we need.
...
mod tests {
    ...
    async fn test_default_policy_leader_aware_routing_with_dc_preference() {
        ...
        let tests = [
            ...
            Test {
                ...
                expected_groups: ExpectedGroupsBuilder::new()
                    .ordered([C]) // the leader, local DC but remote rack
                    .group([A]) // the other local-DC replica
                    .group([G]) // other local-rack nodes
                    .group([B]) // other local-DC nodes
                    .build(),
            },
        ];
    ...
    //   -> still routed deterministically: the ordering depends on the keyspace's consistency
    //      mode alone, not on whether the mapping carried a version.
    //
