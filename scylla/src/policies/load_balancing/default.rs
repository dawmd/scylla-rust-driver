...
impl DefaultPolicy {
    ...
    /// place it appears is the `TABLETS_ROUTING_V2` response payload, whose replica list the
    /// server rotates so that the leader comes first. The driver caches that list in payload
    /// order, which is what lets [`Self::leader_to_prefer`] recover the leader. Two conditions
    /// must hold:
    ...
    fn leader_to_prefer<'a>(
        ...
        // listed them, which the server rotates to put the leader first, and `ReplicasOrdered`
