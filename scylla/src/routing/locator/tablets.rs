...
//! surfaces is the `tablets-routing-v2` payload, whose replica list the server rotates so that
//! the leader comes first. The driver stores the replicas in that payload order, so `replicas[0]`
//! of a cached tablet is the leader, and the built-in load balancing policy uses it to route
//! leader-requiring requests straight there, saving the extra coordinator-to-leader hop.
//!
