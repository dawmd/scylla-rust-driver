...
/// included whenever the driver contacted a node that does not own the tablet.
...
async fn test_tablets_routing_v2_execute_carries_block_and_converges() {
    ...
    let res = test_with_3_node_cluster(
        ...
        |proxy_uris, translation_map, mut running_proxy| async move {
            ...
            // Phase 1, run once: warm the cache. Sequential on purpose -- convergence is a
            // feedback loop (each response teaches the driver the version that the next request
            // must echo), so firing these concurrently would have most of them in flight before
            // the first answer landed and prove nothing.
...
async fn test_tablets_routing_v1_used_when_v2_unavailable() {
    ...
    let res = test_with_3_node_cluster(
        ...
        |proxy_uris, translation_map, mut running_proxy| async move {
            ...
            // contacts a node that does not own it, so from a cold cache a few requests suffice
            // to converge. Sequential, because that convergence is a feedback loop.
            const WARMUP: usize = 12;
            for _ in 0..WARMUP {
                session.execute_unpaged(&select, (PK,)).await.unwrap();
            }
            ...
            while let Ok((frame, _shard)) = rx_resp.try_recv() {
                saw_v1_payload |=
                    response_carries_tablets_payload(frame, CUSTOM_PAYLOAD_TABLETS_V1_KEY);
            }
            assert!(
                saw_v1_payload,
